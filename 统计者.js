				...
				if (m * 100 + d < (today.getMonth() + 1) * 100 + today.getDate()) {
					...
					try {
						const result0 = await bot.api.get({
							...
							list: "usercontribs",
							uclimit: "max",
							ucstart: `2022-${String(m).padStart(2, 0)}-${String(d).padStart(2, 0)}T16:00:00.000Z`,
							ucend: `${d - 1 == 0 ? `2022-${String(m-1).padStart(2, 0)}-${String(days[m-1]).padStart(2, 0)}T16:00:00.000Z` : `2022-${String(m).padStart(2, 0)}-${String(d - 1).padStart(2, 0)}T16:00:00.000Z`}`,
							ucuser: "鬼影233",
							ucprop: ""
						})
						key += `\n"2022年${m}月${d}日",`
