...
const pageList = ["模块:Sandbox", "模块:Sandbox/test"];
bot.login().then(async () => {
	...
	try {
		const result1 = await bot.doEdit({
			...
			text: '._addText {\n\tcontent: "{{:H:沙盒}}";\n}',
			summary:
				"沙盒清理作业，若想保留较长时间，可以在[[Special:我的用户页/Sandbox/styles.css|个人测试区]]作测试，或者翻阅历史记录。",
			tags: "Bot",
			Bot: true,
		});
		console.log(
