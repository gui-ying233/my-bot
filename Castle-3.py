...
def getSkillInfo(description: str):
    ...
    if (skill <= 2):
        attrib = "".join(findall(f"(?='''技能{skill}（精英{skill-1}开放）''')([\s\S]*)(?<='''技能{skill+1}（精英{skill}开放）''')", result1, flags=S)) if (findall(
            f"(?='''技能{skill}（精英{skill-1}开放）''')([\s\S]*)(?<='''技能{skill+1}（精英{skill}开放）''')", result1, flags=S)) else "".join(findall(f"(?='''技能{skill}（精英{skill-1}开放）''')([\s\S]*)(?<=\=\=后勤技能\=\=)", result1, flags=S))
    elif (skill == 3):
        attrib = "".join(findall(
            f"(?='''技能{skill}（精英{skill-1}开放）''')([\s\S]*)(?<=\=\=后勤技能\=\=)", result1, flags=S))
    return attrib
...
for skil in range(0, 3):
    for i in range(0, 10):
        if i >= 7:
            ...
            lv = str(i+1)
        attribTable[skil].append(getDescription(
            serial="技能" + str(skil+1), skLv=lv))
    while (match(".*?{{color\|#0098DC\|(.*?)}}", attribTable[skil][0])):
        for a in range(0, 10):
            variable.append(
                search(".*?{{color\|#0098DC\|(.*?)}}", attribTable[skil][a]).group(1))
        if not variable[0] == variable[9]:
            for _ in range(0, 10):
                attribTable[skil][_] = sub(
                    "(.*?){{color\|#0098DC\|.*?}}", r"\1{{明日方舟技能条|color=blue|"+"|".join(variable) + "}}", attribTable[skil][_], 1)
        else:
            for _ in range(0, 10):
                attribTable[skil][_] = sub(
                    "(.*?{{color\|)#0098DC(\|.*?}})", r"\1blue\2", attribTable[skil][_], 1)
        del variable
        variable = []
    while (match(".*?{{color\|#F49800\|(.*?)}}", attribTable[skil][0])):
        for a in range(0, 10):
            variable.append(
                search(".*?{{color\|#F49800\|(.*?)}}", attribTable[skil][a]).group(1))
        if not variable[0] == variable[9]:
            for _ in range(0, 10):
                attribTable[skil][_] = sub(
                    "(.*?){{color\|#F49800\|.*?}}", r"\1{{明日方舟技能条|color=orange|"+"|".join(variable) + "}}", attribTable[skil][_], 1)
        else:
            for _ in range(0, 10):
                attribTable[skil][_] = attribTable[skil][_].replace(
                    "#F49800", "orange")
        del variable
        variable = []
技能范围 = [[], [], []]
for i in range(3):
    技能范围[i] = "<br />'''技能持续期间攻击范围为：'''{{akrange|" + \
        攻击范围[getScale(i+1)] + "}}" if getScale(i+1) else ""
...
if (r(r"'''技能3（精英2开放）'''.+?\|技能1消耗=(\d*?)\n\|技能1持续=")):
    ...
    if len(set(技能3消耗)) == 1:
        ...
        技能3技力 += '{{akspan|消耗}} {{明日方舟技能条|color=blue|' + \
            '|'.join(技能3消耗)+'}} '
特性 = sub(r'{{color\|#00B0FF\|(.*?)}}', r'<span class="bluetext">\1</span>',
         r(r'\|特性=(.+?)\n\|稀有度=\d+?\n\|职业=.+?\n\|分支=.+?\n'))
...
modtext = search("\=\=模组\=\=\n([\S\s]*?)\<section end=专属模组\s/\>", result1).group(
    1) if search("(\=\=模组\=\=[\S\s]*)\=\=相关道具\=\=", result1) else ""
modtext2 = search("\<section end=专属模组\s/\>\n([\S\s]*?)\<section end=专属模组\s/\>", result1).group(
    1) if search("\<section end=专属模组\s/\>\n([\S\s]*?)\<section end=专属模组\s/\>", result1) else ""
...
info1 = "<poem>\n" + sub("<br(\s?/)?>", "\n",
                         info1).strip("\n\"") + "\n</poem>\n" if info1 else ""
name = search("\|名称=(.*\n)(?=\|类型)",
              modtext).group(1) if search("\|名称=(.*\n)(?=\|类型)", modtext) else ""
name2 = search("\|名称=(.*\n)(?=\|类型)",
               modtext2).group(1) if search("\|名称=(.*\n)(?=\|类型)", modtext2) else ""
mtype = search(
...
task4 = sub("\[\[|]]", "", task4)
level = search("\|解锁等级=(\d{2})", modtext).group(
    1) if search("\|解锁等级=(\d{2})", modtext) else ""
time = 1
unlock = unlock2 = ""
while time < 7 and search("\|材料消耗=" + material + "\s?" + material + "\s?" + cost, modtext):
    unlock += search("\|材料消耗=" + material + "\s?" +
                     material + "\s?" + cost, modtext).group(time)
    if time % 2 == 1:
        unlock += "*"
    elif time < 6:
        unlock += "+"
...
if (modtext2):
    while time < 7 and search("\|材料消耗=" + material + "\s?" + material + "\s?" + cost, modtext2):
        unlock2 += search("\|材料消耗=" + material + "\s?" +
                          material + "\s?" + cost, modtext2).group(time)
        if time % 2 == 1:
            unlock2 += "*"
        elif time < 6:
            unlock2 += "+"
...
while time < 9 and search("\|材料消耗2=" + material + "\s?" + material + "\s?" + material + "\s?" + cost, modtext) and search("\|材料消耗3=" + material + "\s?" + material + "\s?" + material + "\s?" + cost, modtext):
    upgrade1 += search("\|材料消耗2=" + material + "\s?" + material +
                       "\s?" + material + "\s?" + cost, modtext).group(time)
    upgrade2 += search("\|材料消耗3=" + material + "\s?" + material +
                       "\s?" + material + "\s?" + cost, modtext).group(time)
    if time % 2 == 1:
        upgrade1 += "*"
        upgrade2 += "*"
...
if (modtext2):
    while time < 9 and search("\|材料消耗2=" + material + "\s?" + material + "\s?" + material + "\s?" + cost, modtext2) and search("\|材料消耗3=" + material + "\s?" + material + "\s?" + material + "\s?" + cost, modtext2):
        upgrade3 += search("\|材料消耗2=" + material + "\s?" + material +
                           "\s?" + material + "\s?" + cost, modtext2).group(time)
        upgrade4 += search("\|材料消耗3=" + material + "\s?" + material +
                           "\s?" + material + "\s?" + cost, modtext2).group(time)
        if time % 2 == 1:
            upgrade3 += "*"
            upgrade4 += "*"
...
feature = search("\|特性=(.*)", modtext).group(1).rstrip(
    "|特性追加=yes").rstrip("|特性追加=no") if search("\|特性=(.*)", modtext) else ""
...
if (modtext2):
    ...
    feature2 = search("\|特性=(.*)", modtext2).group(1).rstrip(
        "|特性追加=yes").rstrip("|特性追加=no") if search("\|特性=(.*)", modtext2) else ""
...
mod = "".join(["""== 模组 ==\n{{明日方舟模组\n|干员名=""", 代号, """\n|职业分支=""", branch, """|模组名=""", name, """|模组类型=""", mtype, """|证章信息=""", info1, """|模组信息=""", info2, """|模组任务-1=①：""", task1, """|模组任务-2=②：""", task2, """|解锁需求=精英阶段2 """, level,
              """级，信赖值达到100%，完成该模组所有模组任务\n""", """|解锁消耗={{#invoke:明日方舟材料|calc|""", unlock, """}}\n""", """|升级消耗-1={{#invoke:明日方舟材料|calc|""", upgrade1, """}}\n""", """|升级消耗-2={{#invoke:明日方舟材料|calc|""", upgrade2, """}}\n""", """|基础数值变化-1="""])
if hp1:
...
if search("\|模组名\=\|模组类型", mod):
    ...
    if search("特性追加=yes", modtext):
        特性 = '<span class="talentblock">' + 代号 + '证章</span><br />' + 特性 + '<br /><span class="talentblock">' + \
            name.rstrip('\n') + '</span><br />' + 特性 + '<br />' + \
            sub(r'{{color\|blue\|(.*?)}}',
                r'<span class="bluetext">\1</span>', feature)
    else:
        特性 = '<span class="talentblock">' + 代号 + '证章</span><br />' + 特性 + '<br /><span class="talentblock">' + \
            name.rstrip('\n') + '</span><br />' + \
            sub(r'{{color\|blue\|(.*?)}}',
...
if modtext2:
    if search("特性追加=yes", modtext):
        特性 += '<br />' + '<span class="talentblock">' + name2.rstrip('\n') + '</span><br />' + sub(r'{{color\|#00B0FF\|(.*?)}}', r'<span class="bluetext">\1</span>', r(
            r'\|特性=(.+?)\n\|稀有度=\d+?\n\|职业=.+?\n\|分支=.+?\n')) + '<br />' + sub(r'{{color\|blue\|(.*?)}}', r'<span class="bluetext">\1</span>', feature2)
    else:
        特性 += '<br />' + '<span class="talentblock">' + name2.rstrip('\n') + '</span><br />' + sub(
            r'{{color\|blue\|(.*?)}}', r'<span class="bluetext">\1</span>', feature2)
output1 = '''{{标题格式化}}
{{明日方舟:导航}}'''+异格+'''
...
for k, v in 术语字典.items():
    output1 = sub(r"{{术语\|"+k.replace(".", "\.")+r"\|({{color\|(blue|orange)\|"+v+"}}|"+v+")}}",
                  r'<span style="border-bottom:1px solid;">\1</span><ref name="'+v+'">'+术语释义["{{术语|"+k+"|"+v+"}}"]+'</ref>', output1, 1)
    # output1 = output1.replace(k, f'<span style="border-bottom:1px solid;">{v}</span><ref name="{v}">' + 术语释义[k] + '</ref>', 1)
    output1 = sub(r"{{术语\|"+k.replace(".", "\.")+r"\|({{color\|(blue|orange)\|"+v+"}}|"+v+")}}",
                  r'<span style="border-bottom:1px solid;">\1</span><ref name="'+v+'" />', output1)
