...
class MediaWikiJS {
    ...
    constructor(options) {
        if (!options) {
            throw new MediaWikiJSError_1.MediaWikiJSError('NO_CONFIG');
        }
        this.api = new API_1.API(this, options);
    ...
    async login(username, password) {
        if (username && !this.options.botUsername)
            this.options.botUsername = username;
        if (password && !this.options.botPassword)
            this.options.botPassword = password;
        if (!username && this.options.botUsername)
            username = this.options.botUsername;
        if (!password && this.options.botPassword)
            password = this.options.botPassword;
        if (!username || !password)
            throw new MediaWikiJSError_1.MediaWikiJSError('NO_CREDENTIALS');
        ...
        if (actionLogin?.login?.result)
            throw new MediaWikiJSError_1.MediaWikiJSError('FAILED_LOGIN', actionLogin?.login?.result);
        // Unspecified throwing
        throw new MediaWikiJSError_1.MediaWikiJSError('FAILED_LOGIN', 'Unspecified error! Dumping: ', JSON.stringify(actionLogin));
    }
    /**
     ...
     */
    async getCSRFToken() {
        let tokenPack = await this.api.get({
            action: 'query',
            meta: 'tokens',
            type: 'csrf'
        });
    ...
    async getPagesInCategory(category, onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(body.query.categorymembers);
    ...
    async getArticleCategories(title, onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(page.categories);
    ...
    async search(keyword, onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(body.query.search);
    ...
    async getUserContribs({ user, start, namespace = '', onlyTitles = false }) {
        ...
        if (onlyTitles)
            this.getList(body.query.usercontribs);
    ...
    async getImages(start, onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(body.query.allimages);
    ...
    async getImagesFromArticle({ page, onlyTitles = false, otherOptions = {} }) {
        ...
        if (onlyTitles)
            return this.getList(article.images);
    ...
    async getImageUsage(fileName, onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(body.query.imageusage);
    ...
    async getRecentChanges(start = '', onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(body.query.recentchanges);
    ...
    async getQueryPage(queryPage, onlyTitles = false) {
        ...
        if (onlyTitles)
            return this.getList(body.query.querypage.results);
     ...
     * @param onlyTitles - Whether to only list the page titles.
     */
    async getBackLinks(page, onlyTitles = false) {
        const body = await this.api.get({
            action: 'query',
            list: 'backlinks',
            blnamespace: 0,
        ...
        if (onlyTitles)
            return this.getList(body.query.backlinks);
        return body.query.backlinks;
    }
}
exports.MediaWikiJS = MediaWikiJS;
